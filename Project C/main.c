...
static void print_header(FILE *csv) {
    fprintf(csv, "method,param,psnr,ssim,time_ms,estimated_bytes,compression_ratio,compression_percent,file\n");
}
...
static void run_jpeg_series(const Image *img, FILE *csv, long original_bytes) {
    ...
    for (int i = 0; i < quality_count; i++) {
        ...
        printf("q=%d | PSNR=%.4f | SSIM=%.6f | time=%.2f ms | est=%ld bytes | ratio=%.2f\n",
               quality, current_psnr, current_ssim, time_ms, stats.estimated_bytes, ratio);

        fprintf(csv, "JPEG-like,q=%d,%.4f,%.6f,%.2f,%ld,%.2f,%.2f,%s\n",
...
static void run_svd_series(const Image *img, FILE *csv, long original_bytes) {
    ...
    if (img->width > 160 || img->height > 160) {
        ...
        fprintf(csv, "SVD,skipped,0,0,0,0,0,0,image too large\n");
        return;
    }
    ...
    for (int i = 0; i < k_count; i++) {
        ...
        printf("k=%d | PSNR=%.4f | SSIM=%.6f | time=%.2f ms | est=%ld bytes | ratio=%.2f\n",
...
int main(int argc, char **argv) {
    ...
    printf("Оригинальный объём RGB-данных: %ld bytes\n", original_bytes);
